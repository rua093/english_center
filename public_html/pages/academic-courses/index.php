...
$courses = $academicModel->listCoursesPage($coursePage, $coursePerPage);
                                ...
                                <td>
                                    <span class="inline-flex flex-wrap items-center gap-2">
                                        ...
                                        <?php if ($canUpdateCourse): ?>
                                            <a
                                                href="<?= e(page_url('courses-academic', ['edit' => $courseId, 'course_page' => $coursePage, 'course_per_page' => $coursePerPage])); ?>"
                                                class="admin-action-icon-btn"
                                        ...
                                        <?php if ($canDeleteCourse): ?>
                                            <form class="inline-block" method="post" action="/api/courses/delete?id=<?= $courseId; ?>&course_page=<?= (int) $coursePage; ?>&course_per_page=<?= (int) $coursePerPage; ?>" onsubmit="return confirm('Bạn có chắc muốn xóa khóa học này không?');">
                                                <?= csrf_input(); ?>
                                                <button
            ...
            <?php if ($courseTotal > 0): ?>
                <div class="border-t border-slate-200 bg-slate-50/80 px-3 py-2">
                    <div class="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-600">
                        <span class="font-medium">Trang <?= (int) $coursePage; ?>/<?= (int) $courseTotalPages; ?> - Tổng <?= (int) $courseTotal; ?> khóa học</span>
                        <div class="inline-flex items-center gap-1.5">
                            <form class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2 py-1" method="get" action="<?= e(page_url('courses-academic')); ?>">
                                ...
                                <select id="course-per-page" name="course_per_page" class="h-7 rounded-md border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-700" onchange="this.form.submit()">
                            ...
                            <?php if ($coursePage > 1): ?>
                                <a class="inline-flex h-7 items-center rounded-md border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-700 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700" href="<?= e(page_url('courses-academic', ['course_page' => $coursePage - 1, 'course_per_page' => $coursePerPage])); ?>">Trước</a>
                            <?php else: ?>
                            ...
                            <?php if ($coursePage < $courseTotalPages): ?>
                                <a class="inline-flex h-7 items-center rounded-md border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-700 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700" href="<?= e(page_url('courses-academic', ['course_page' => $coursePage + 1, 'course_per_page' => $coursePerPage])); ?>">Sau</a>
                            <?php else: ?>
