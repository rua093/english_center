...
$classStatusMap = [
    'upcoming' => t('my_classes.status_upcoming'),
    'active' => t('my_classes.active'),
    'graduated' => t('my_classes.completed'),
    'cancelled' => t('my_classes.cancelled'),
];
$classStatus = $classStatusMap[(string) ($classRow['class_status'] ?? '')] ?? t('my_classes.active');

$classDetail = [
    'name' => (string) ($classRow['class_name'] ?? ''),
    'teacher' => (string) ($classRow['teacher_name'] ?? ''),
    'status' => $classStatus,
    'schedule' => $firstSchedule ? (date('d/m/Y', strtotime((string) ($firstSchedule['study_date'] ?? ''))) . ' ' . date('H:i', strtotime((string) ($firstSchedule['start_time'] ?? ''))) . ' - ' . date('H:i', strtotime((string) ($firstSchedule['end_time'] ?? '')))) : t('my_classes.no_schedule'),
    ...
    'room' => $firstSchedule ? trim((string) ($firstSchedule['room_name'] ?? 'Online')) : t('my_classes.no_room'),
...
$tuitionStatus = $tuitionRow === [] ? t('my_classes.no_tuition') : (((string) ($tuitionRow['status'] ?? 'debt')) === 'paid' ? t('my_classes.completed') : t('my_classes.debt'));
...
foreach ($assignmentRows as $assignmentRow) {
    ...
    $isGraded = $score !== null;
    $status = t('my_classes.not_submitted');
    $color = 'slate';

    if ($submittedAt !== '') {
        if ($isGraded) {
            $status = t('my_classes.graded');
            $color = 'emerald';
            $assignmentScores[] = $score;
        } else {
            $status = t('my_classes.pending_grade');
            $color = 'blue';
        }
    }
    ...
    $assignments[] = [
        ...
        'is_graded' => $isGraded,
        'disabled_reason' => $isGraded ? t('my_classes.graded_no_resubmit_short') : ($isExpired ? t('my_classes.expired_no_submit') : ''),
    ];
}
...
foreach ($examRows as $examRow) {
    ...
    $exams[] = [
        ...
        'date' => !empty($examRow['exam_date']) ? date('d/m/Y', strtotime((string) $examRow['exam_date'])) : t('my_classes.not_happened'),
...
<section class="min-h-screen bg-[radial-gradient(circle_at_top,_#ffffff_0%,_#f2f6fb_32%,_#e3ebf4_100%)] pb-20 font-jakarta relative overflow-hidden">
    ...
    <div class="relative z-10 pt-6 px-4 sm:px-8 max-w-7xl mx-auto flex justify-between items-center">
        <a class="group inline-flex items-center gap-2 rounded-full bg-white backdrop-blur-md border border-slate-300 px-5 py-2.5 text-xs font-bold text-slate-800 shadow-lg shadow-slate-300/60 transition-all hover:-translate-y-0.5 hover:bg-white hover:text-emerald-700" href="<?= e(page_url('classes-my')); ?>">
            <i class="fa-solid fa-arrow-left transition-transform group-hover:-translate-x-1"></i> <?= e(t('my_classes.back_to_list')); ?>
        </a>
    </div>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 relative z-10 mt-6 space-y-6">
        ...
        <div class="glass-card rounded-[2rem] p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border border-slate-200" data-aos="fade-up">
            <div class="flex items-center gap-5">
                ...
                <div>
                    ...
                    <p class="text-sm font-bold text-slate-500 flex items-center gap-4">
                        <span><i class="fa-regular fa-user mr-1 text-slate-400"></i> <?= e(t('my_classes.teacher_short')); ?>: <span class="text-slate-700"><?= e($classDetail['teacher']) ?></span></span>
                    ...
                    <p class="mt-1 text-xs font-semibold text-slate-500">
                        <span class="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-2.5 py-1 shadow-sm">
                            ...
                            <span><?= e(t('my_classes.study_time')); ?>: <?= e($classDetail['start_date']) ?> - <?= e($classDetail['end_date']) ?></span>
                        </span>
                    </p>
            ...
            <a href="#class-assignments" class="group inline-flex items-center gap-3 rounded-[1.25rem] border border-rose-300 bg-white px-5 py-4 text-left text-slate-900 shadow-2xl shadow-rose-200/50 transition-all hover:-translate-y-1 hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-rose-300 focus:ring-offset-2 focus:ring-offset-white">
                ...
                <span class="min-w-0 leading-tight">
                    <span class="block text-[10px] font-black uppercase tracking-[0.35em] text-rose-500"><?= e(t('my_classes.class_assignments')); ?></span>
                    <span class="mt-1 block text-sm font-black tracking-tight text-slate-900"><?= e(t('my_classes.view_assignments_now')); ?></span>
                </span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch" data-aos="fade-up" data-aos-delay="100">
            <div class="relative h-full overflow-hidden rounded-[2rem] border border-emerald-200 bg-white p-6 shadow-2xl shadow-emerald-200/60 transition-all hover:-translate-y-0.5 hover:shadow-[0_28px_70px_rgba(34,197,94,0.24)]">
                ...
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1"><?= e(t('my_classes.attendance_rate')); ?></p>
                ...
                <div class="flex gap-3 flex-1">
                    <div class="flex-1 bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-center shadow-sm">
                        ...
                        <p class="text-[9px] font-bold text-emerald-700 uppercase"><?= e(t('my_classes.present')); ?></p>
                    </div>
                    <div class="flex-1 bg-rose-50 border border-rose-200 rounded-xl p-3 text-center shadow-sm">
                        ...
                        <p class="text-[9px] font-bold text-rose-700 uppercase"><?= e(t('my_classes.absent')); ?></p>
                    </div>
                </div>
                <p class="mt-3 text-[11px] font-semibold text-slate-500"><?= e(t('my_classes.total_sessions', ['count' => (string) $attendanceTotal])); ?></p>
                </div>
            </div>
            ...
            <div class="relative h-full overflow-hidden rounded-[2rem] border border-amber-200 bg-white p-6 shadow-2xl shadow-amber-200/60 transition-all hover:-translate-y-0.5 hover:shadow-[0_28px_70px_rgba(245,158,11,0.24)]">
                ...
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1"><?= e(t('my_classes.course_tuition')); ?></p>
                        ...
                        <p class="mt-1 text-[11px] font-semibold text-slate-500"><?= e(t('my_classes.tuition_remaining_percent', ['amount' => number_format($tuitionRemaining), 'percent' => (string) (100 - $tuitionPaidPercent)])); ?></p>
                    </div>
                ...
                <div class="space-y-4 flex-1">
                    <div>
                        <div class="mb-2 flex items-center justify-between gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
                            <span><?= e(t('my_classes.payment_progress')); ?></span>
                        ...
                        <div class="mt-2 flex items-center justify-between text-[11px] font-semibold text-slate-500">
                            <span><?= e(t('my_classes.paid_amount', ['amount' => number_format($tuitionPaid)])); ?></span>
                            <span><?= e(t('my_classes.remaining_amount', ['amount' => number_format($tuitionRemaining)])); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400"><?= e(t('my_classes.activities')); ?></p>
                                <p class="mt-1 text-sm font-bold text-slate-700"><?= e(t('my_classes.registered_activity_count', ['count' => (string) $activityRegisteredCount])); ?></p>
                                <p class="mt-1 text-[11px] text-slate-500"><?= e(t('my_classes.activity_paid_copy')); ?></p>
                            </div>
                            <div class="text-right">
                                ...
                                <p class="text-[11px] font-semibold text-slate-500"><?= e(t('my_classes.remaining_amount', ['amount' => number_format($activityFeeRemaining)])); ?></p>
                            </div>
                        </div>
                        ...
                        <div class="mt-2 flex items-center justify-between text-[11px] font-semibold text-slate-500">
                            <span><?= e(t('my_classes.paid_percent', ['percent' => (string) $activityFeePercent])); ?></span>
                            <span><?= e(t('my_classes.unpaid_percent', ['percent' => (string) (100 - $activityFeePercent)])); ?></span>
                        </div>
                    </div>

                    <div class="flex items-center justify-between">
                        <?php if ($tuitionStatus === t('my_classes.debt')): ?>
                            <span class="bg-rose-100 text-rose-700 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border border-rose-200"><?= e(t('my_classes.tuition_debt')); ?></span>
                            <a href="#" class="text-xs font-bold text-rose-600 hover:underline"><?= e(t('my_classes.pay_now')); ?> <i class="fa-solid fa-arrow-right text-[10px]"></i></a>
                        <?php elseif ($tuitionStatus === t('my_classes.completed')): ?>
                            <span class="bg-emerald-100 text-emerald-700 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border border-emerald-200"><?= e(t('my_classes.completed')); ?></span>
                        <?php else: ?>
            ...
            <div class="relative h-full overflow-hidden rounded-[2rem] border border-blue-200 bg-white p-6 shadow-2xl shadow-blue-200/60 transition-all hover:-translate-y-0.5 hover:shadow-[0_28px_70px_rgba(59,130,246,0.24)] flex flex-col justify-between">
                ...
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1"><?= e(t('my_classes.average_assignment_score')); ?></p>
                ...
                <div class="bg-slate-50 p-3 rounded-xl border border-slate-200 mt-auto shadow-sm">
                    <?php if ($averageAssignmentScore !== '--'): ?>
                        <p class="text-xs font-medium text-slate-600"><?= e(t('my_classes.evaluation')); ?>: <span class="font-black text-emerald-600"><?= e(t('my_classes.based_on_graded')); ?></span></p>
                        <p class="text-[10px] text-slate-400 mt-1"><?= e(t('my_classes.average_score_copy')); ?></p>
                    <?php else: ?>
                        <p class="text-xs font-medium text-slate-600"><?= e(t('my_classes.no_graded_assignment')); ?></p>
                        <p class="text-[10px] text-slate-400 mt-1"><?= e(t('my_classes.no_graded_copy')); ?></p>
                    <?php endif; ?>
                </div>
                </div>
            </div>
        ...
        <div id="class-assignments" data-assignment-panel="1" class="scroll-mt-24 md:scroll-mt-8 rounded-[2rem] border border-slate-200 bg-white shadow-2xl shadow-slate-300/60 overflow-visible transition-all duration-300">
            <div class="p-6 md:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h2 class="text-lg font-black text-slate-800 flex items-center gap-3">
                    ...
                    <?= e(t('my_classes.assignment_list')); ?>
                </h2>
                ...
                <span class="inline-flex group relative" <?= !empty($defaultHomework['disabled_reason']) ? 'title="' . e((string) $defaultHomework['disabled_reason']) . '"' : ''; ?>>
                    <button type="button" class="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 via-sky-600 to-cyan-600 px-5 py-2.5 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-blue-500/25 transition-all hover:-translate-y-0.5 hover:shadow-blue-500/40 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none" data-homework-open="1" data-homework-class="<?= e($classDetail['name']); ?>" data-homework-assignment-id="<?= (int) ($defaultHomework['id'] ?? 0); ?>" data-homework-assignment="<?= e((string) ($defaultHomework['title'] ?? '')); ?>" data-homework-deadline="<?= e((string) ($defaultHomework['deadline_raw'] ?? '')); ?>" data-homework-note="<?= e((string) ($defaultHomework['note'] ?? '')); ?>" data-homework-status="<?= e((string) ($defaultHomework['status'] ?? '')); ?>" data-homework-empty="<?= empty($assignments) ? '1' : '0'; ?>" <?= $defaultHomeworkCanOpen ? '' : 'disabled'; ?>>
                        <i class="fa-solid fa-plus"></i> <?= e(!empty($defaultHomework['can_resubmit']) ? t('my_classes.resubmit') : t('my_classes.submit_new')); ?>
                    </button>
            ...
            <div class="max-h-[70vh] md:max-h-[640px] overflow-y-auto overflow-x-hidden">
                <div class="space-y-3 p-3 md:hidden">
                    <?php if (empty($pagedAssignments)): ?>
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-8 text-center text-sm font-semibold text-slate-500">
                            <?= e(t('my_classes.no_class_assignment')); ?>
                        </div>
                    <?php else: ?>
                            ...
                            <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div class="mt-2 flex flex-wrap gap-2 text-[11px] font-semibold">
                                            ...
                                            <?php else: ?>
                                                <span class="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-slate-400"><?= e(t('my_classes.not_submitted')); ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    <?php
                                ...
                                <?php if (!empty($hw['file_url'])): ?>
                                    <a href="<?= e(normalize_public_file_url((string) $hw['file_url'])); ?>" target="_blank" rel="noopener" class="mt-3 inline-flex items-center gap-1 rounded-full border border-blue-100 bg-blue-50 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest text-blue-700 transition hover:bg-blue-100">
                                        <i class="fa-solid fa-paperclip"></i> <?= e(t('my_classes.download_assignment_file')); ?>
                                    </a>
                                <?php endif; ?>
                ...
                <table class="hidden w-full table-modern table-fixed text-left md:table">
                    ...
                    <tbody>
                        <?php if (empty($pagedAssignments)): ?>
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-sm font-semibold text-slate-500"><?= e(t('my_classes.no_class_assignment')); ?></td>
                        </tr>
                        <?php else: ?>
                        <?php foreach($pagedAssignments as $hw): ?>
                        <tr>
                ...
                <div data-assignment-pagination="1" class="border-t border-slate-200 bg-slate-50/80 px-4 py-3 sm:px-6">
                    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div class="text-xs font-semibold text-slate-500">
                            <?= e(t('my_classes.assignment_page_info', ['current' => (string) $assignmentPage, 'total' => (string) $assignmentTotalPages, 'count' => (string) $assignmentTotal])); ?>
                        </div>
                        <form class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-2" method="get" action="<?= e(page_url('classes-my-details', ['class_id' => $selectedClassId])); ?>" data-assignment-per-page-form="1">
                            ...
                            <label for="assignment-per-page" class="text-xs font-semibold text-slate-500"><?= e(t('my_classes.rows')); ?></label>
                        ...
                        <div class="grid grid-cols-2 gap-2 text-xs font-semibold text-slate-600 sm:flex sm:flex-wrap sm:items-center">
                            <?php if ($assignmentPage > 1): ?>
                                <a class="inline-flex h-10 items-center justify-center rounded-md border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700 sm:h-8" href="<?= e(page_url('classes-my-details', ['class_id' => $selectedClassId, 'assignment_page' => $assignmentPage - 1, 'assignment_per_page' => $assignmentPerPage])); ?>" data-assignment-page-link="1"><?= e(t('my_classes.previous')); ?></a>
                            <?php else: ?>
                                <span class="inline-flex h-10 items-center justify-center rounded-md border border-slate-200 bg-slate-100 px-3 text-xs font-semibold text-slate-400 sm:h-8"><?= e(t('my_classes.previous')); ?></span>
                            <?php endif; ?>

                            <?php if ($assignmentPage < $assignmentTotalPages): ?>
                                <a class="inline-flex h-10 items-center justify-center rounded-md border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700 sm:h-8" href="<?= e(page_url('classes-my-details', ['class_id' => $selectedClassId, 'assignment_page' => $assignmentPage + 1, 'assignment_per_page' => $assignmentPerPage])); ?>" data-assignment-page-link="1"><?= e(t('my_classes.next')); ?></a>
                            <?php else: ?>
                                <span class="inline-flex h-10 items-center justify-center rounded-md border border-slate-200 bg-slate-100 px-3 text-xs font-semibold text-slate-400 sm:h-8"><?= e(t('my_classes.next')); ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="rounded-[2rem] border border-slate-200 bg-white shadow-2xl shadow-slate-300/60 overflow-hidden">
            <div class="p-6 md:p-8 border-b border-slate-100 flex items-center gap-3">
                ...
                <h2 class="text-lg font-black text-slate-800"><?= e(t('my_classes.exam_results')); ?></h2>
            </div>
            ...
            <div class="overflow-visible">
                <table class="w-full table-modern table-fixed text-left">
                    <thead>
                        <tr>
                            <th class="w-[28%]"><?= e(t('my_classes.exam')); ?></th>
                            <th class="w-[14%]"><?= e(t('my_classes.exam_date')); ?></th>
                            <th class="w-[9%] text-center" title="Listening"><?= e(t('my_classes.listening')); ?></th>
                            <th class="w-[9%] text-center" title="Reading"><?= e(t('my_classes.reading')); ?></th>
                            <th class="w-[9%] text-center" title="Writing"><?= e(t('my_classes.writing')); ?></th>
                            <th class="w-[9%] text-center" title="Speaking"><?= e(t('my_classes.speaking')); ?></th>
                    ...
                    <tbody>
                        <?php if (empty($exams)): ?>
                        <tr>
                            <td colspan="7" class="px-4 py-8 text-center text-sm font-semibold text-slate-500"><?= e(t('my_classes.no_exam_result')); ?></td>
                        </tr>
                        <?php else: ?>
                        ...
                        <tr>
                            <td class="align-top">
                                ...
                                <p class="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-400"><?= e($ex['type'] === 'entry' ? t('my_classes.exam_entry') : ($ex['type'] === 'periodic' ? t('my_classes.exam_periodic') : ($ex['type'] === 'final' ? t('my_classes.exam_final') : $ex['type']))); ?></p>
                            </td>
                            <td class="align-top">
            ...
            (function () {
                ...
                async function loadAssignmentPanel(url, pushState) {
                    ...
                    try {
                        ...
                        if (!response.ok) {
                            throw new Error(myClassesI18n.loadAssignmentsFailed);
                        }
                        ...
                        if (!(incomingPanel instanceof HTMLElement)) {
                            throw new Error(myClassesI18n.newAssignmentListNotFound);
                        }
                    ...
                    if (openButton instanceof HTMLElement) {
                        if (openButton.dataset.homeworkEmpty === '1') {
                            if (typeof showNotify === 'function') {
                                showNotify('info', myClassesI18n.noAssignment);
                            }
                            return;
                        }
