...
<div class="grid gap-4">
    ...
    <?php if ($canShowForm): ?>
        <article class="order-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3><?= $editingRoom ? 'Sua phong hoc' : 'Them phong hoc'; ?></h3>
            <form class="grid gap-3 md:grid-cols-2" method="post" action="/api/rooms/save" autocomplete="off">
                ...
                <div class="md:col-span-2 flex flex-wrap items-center gap-2">
                    <button class="<?= ui_btn_primary_classes(); ?>" type="submit">Luu phong hoc</button>
                    <?php if ($editingRoom): ?>
                        <a class="<?= ui_btn_secondary_classes(); ?>" href="<?= e(page_url('rooms-manage')); ?>">Tao moi</a>
                    <?php endif; ?>
                </div>
            </form>
        </article>
            ...
            <table class="min-w-full border-collapse text-sm" data-enable-row-detail="1">
                ...
                <tbody>
                    <?php if (empty($rooms)): ?>
                        <tr>
                            <td colspan="3">
                                <div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-sm text-slate-500">Chua co phong hoc nao.</div>
                            </td>
                        </tr>
                    <?php else: ?>
                                ...
                                <td>
                                    <span class="inline-flex flex-wrap items-center gap-2">
                                        ...
                                        <?php if ($canDeleteRoom): ?>
                                            <form class="inline-block" method="post" action="/api/rooms/delete?id=<?= $roomId; ?>" onsubmit="return confirm('Ban co chac muon xoa phong hoc nay?');">
                                                <?= csrf_input(); ?>
                                                <button
            ...
            <?php if ($roomTotal > 0): ?>
                <div class="border-t border-slate-200 bg-slate-50/80 px-3 py-2">
                    <div class="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-600">
                        <span class="font-medium">Trang <?= (int) $roomPage; ?>/<?= (int) $roomTotalPages; ?> - Tong <?= (int) $roomTotal; ?> phong hoc</span>
                        <div class="inline-flex items-center gap-1.5">
                            <form class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2 py-1" method="get" action="<?= e(page_url('rooms-manage')); ?>">
                                ...
                                <label class="text-[11px] font-semibold text-slate-500" for="room-per-page">So dong</label>
                            ...
                            <?php if ($roomPage > 1): ?>
                                <a class="inline-flex h-7 items-center rounded-md border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-700 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700" href="<?= e(page_url('rooms-manage', ['room_page' => $roomPage - 1, 'room_per_page' => $roomPerPage])); ?>">Truoc</a>
                            <?php else: ?>
