...
$classFilters = ['status' => $statusFilter];
...
$classTotal = $academicModel->countClasses($teacherId, $searchQuery, $classFilters);
$classTotalPages = max(1, (int) ceil($classTotal / $classPerPage));
if ($classPage > $classTotalPages) {
    $classPage = $classTotalPages;
}
$classes = $academicModel->listClassesPage($classPage, $classPerPage, $teacherId, $searchQuery, $classFilters);
...
<div class="grid gap-4">
        ...
        <article
            ...
            data-ajax-page-value="classes-academic"
                            ...
                            <td>
                                <span class="inline-flex flex-wrap items-center gap-2">
                                    <?php if (can_access_page('classrooms-academic')): ?>
                                        <a
                                            href="<?= e(page_url('classrooms-academic', ['course_id' => (int) ($class['course_id'] ?? 0), 'class_id' => (int) $class['id'], 'class_page' => $classPage, 'class_per_page' => $classPerPage, 'search' => $searchQuery !== '' ? $searchQuery : null, 'status' => $statusFilter !== '' ? $statusFilter : null])); ?>"
                                    ...
                                    <?php if ($canUpdateClass): ?>
                                        <a
                                            href="<?= e(page_url('classes-academic-edit', ['id' => (int) $class['id'], 'class_page' => $classPage, 'class_per_page' => $classPerPage, 'search' => $searchQuery !== '' ? $searchQuery : null, 'status' => $statusFilter !== '' ? $statusFilter : null])); ?>"
                                            class="admin-action-icon-btn"
